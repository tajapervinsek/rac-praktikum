...
def priblizek_po_korakih(n, k):
    x = n / 2
    # Postopek ponovimo k-krat, da pridemo do k-tega približka
    # Uporabimo zanko for, ki teče od 0 do k-1
    for _ in range(k):
        x = (x + n / x) / 2
    return x
# =====================================================================@001004=
...
def priblizek_do_natancnosti(n, eps):
    ...
    # Zanko ponavljamo, dokler je razlika med kvadratom x in n 
    # večja ali enaka predpisani natančnosti eps
    while abs(x**2 - n) >= eps:
        x = (x + n / x) / 2
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDAzLCJ1c2VyIjoxMTQ2Nn0:1w2USA:OBZ7aHi1A9dvM2_-90dxzWw_kCtr-Yt4AedqX5T13sQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0IjoxMDA0LCJ1c2VyIjoxMTQ2Nn0:1w2USA:VepEaUO5iAMhYJdiGFJC9ZTnIX-V-G6h1MwVIyYW9pQ"
        try:
