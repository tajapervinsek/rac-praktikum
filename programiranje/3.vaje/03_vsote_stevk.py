...
def vsota_stevk(n):
    vsota = 0
    while n > 0:
        vsota += n % 10  # Vzamemo zadnjo števko
        n //= 10         # Odrežemo zadnjo števko
    return vsota
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODEsInVzZXIiOjExNDY2fQ:1w2Tyl:agxmXrRqfeyQBodXDLyz0URk2cCplvHtyUbExXKgXgQ"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODIsInVzZXIiOjExNDY2fQ:1w2Tyl:2TU60hANf77K4SHc5rh9grMog3cfJ8T2pKCSZD-X7bw"
        try:
            try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5ODMsInVzZXIiOjExNDY2fQ:1w2Tyl:RaRMWO4HgRhj6c4uSJYXYzgOOu8NJXF0FReb54EuU3E"
        try:
