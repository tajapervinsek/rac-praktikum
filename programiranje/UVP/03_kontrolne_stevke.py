...
def dodaj_kontrolno_stevko(sklic):
    ...
    vsota = (2 * s0 + 3 * s1 + 4 * s2 + 5 * s3 + 6 * s4 + 7 * s5 +
             8 * s6 + 9 * s7 + 10 * s8 + 11 * s9 + 12 * s10 + 13 * s11)
    razlika = 11 - vsota % 11
    kontrolna = min(10, razlika) % 10
    return 10 * sklic + kontrolna
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjQsInVzZXIiOjExNDY2fQ:1vusch:YEfRAVYB9D_PCgoYTZdy-WkTwDVjWMUdvdD6-DFXnKA"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjUsInVzZXIiOjExNDY2fQ:1vusch:kSbRPj4uCuKwxslIE-tB6MWfmeD_94njVpRlNFXjL7o"
        try:
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo5MjYsInVzZXIiOjExNDY2fQ:1vusch:nkf0D7UQiY0iten2tdTnvDJGiRizPzR9yaJnIanLTQc"
        try:
